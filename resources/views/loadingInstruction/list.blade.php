          ...
          <div class="modal-dialog">
              <div class="modal-content">
                  ...
                  <div class="modal-body">
                      <p>Czy aby na pewno chcesz archiwizować ten załadunek?</p>
            ...
            <td>
              @if (Auth::user()->ID_roles == 1)
                <span style="float:left; margin-right:3px;">
                  ...
                  {{ Form::close() }}</span>
                <span style="float:left; margin-right:3px;">
                  {{ Form::open(['method' => 'get', 'route' => [ 'loadingInstruction.edit', $loadingInstruction->id ]]) }}
                  {{ Form::button('<i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edytuj dane', ['type' => 'submit', 'class' => 'btn btn-info btn-xs'] )  }}
                  {{ Form::close() }}</span>
                <span style="float:left; margin-right:3px;">
                  {{ Form::open(['method' => 'get', 'route' => [ 'LoadingEdit', $loadingInstruction->id ]]) }}
                  {{ Form::button('<i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edytuj listę załadunkową', ['type' => 'submit', 'class' => 'btn btn-info btn-xs'] )  }}
                  {{ Form::close() }}</span>

                  {!! Form::model($loadingInstruction, ['method' => 'post', 'route' => ['LoadingInstructionToArchive', $loadingInstruction->id], 'class' =>'form-inline form-delete']) !!}

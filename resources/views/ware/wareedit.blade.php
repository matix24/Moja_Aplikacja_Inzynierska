        ...
        <div class="container-fluid">
          <div class="row">
            <ol class="breadcrumb">
              ...
              <li class="active">Edytuj towar</li>
            </ol>
          </div>
            <div class="row" id="main" >
            <div class="panel panel-default">
                <div class="panel-heading"><b>Edycja towaru</b></div>
                <div class="panel-body">
                  {{-- <form class="form-horizontal" role="form" method="POST" action="{{ route('employee.update') }}"> --}}
                      {{ csrf_field() }}
                      {!! Form::model($ware, ['method' => 'PATCH', 'class' => 'form-horizontal', 'action' => ['WareController@update', $ware->id] ]) !!}
                          ...
                          <div class="col-md-6">
                              <select id="ID_packaging" class="form-control" name="ID_packaging" value="{{ old('ID_packaging') }}" required>
                                @foreach ($packagings as $packaging)
                                  @if ($packaging->id === $ware->ID_packaging)
                                    <option value="{{ $packaging->id }}" selected>{{ $packaging->name }}</option>
                                  @else
                          ...
                          <div class="col-md-6">
                              <select id="ID_hardiness" class="form-control" name="ID_hardiness" value="{{ old('ID_hardiness') }}" required>
                                @foreach ($hardinesss as $hardiness)
                                  @if ($hardiness->id === $ware->ID_hardiness)
                                    <option value="{{ $hardiness->id }}" selected>{{ $hardiness->name }}</option>
                                  @else
                      ...
                      <div class="form-group{{ $errors->has('weight_of_package') ? ' has-error' : '' }}">
                          ...
                          <div class="col-md-6">
                              {{-- <input id="name" type="text" class="form-control" name="name" value="{{ old('name') }}" required autofocus> --}}
                              {!! Form::text('weight_of_package', null, ['class' => 'form-control']) !!}
